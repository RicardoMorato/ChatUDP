...
def find_sum_checksum(message: bytes) -> str:
    ...
    checksum_array = [
        message_bits[i : i + slice_length]
        for i in range(0, len(message_bits), slice_length)
    ]
...
def find_checksum(message: bytes) -> str:
    ...
    if len(sum_checksum) > 16:
        ...
        sum_checksum = bin(
            int(sum_checksum[:overflow], 2) + int(sum_checksum[overflow:], 2)
        )[2:]
...
def extract_data_and_checksum(message_with_checksum):
    if len(message_with_checksum) < 2:
        raise ValueError(
            "Dados são menores que o comprimento esperado para o checksum."
        )

    checksum_length = 2  # comprimento do checksum em bytes
    data = message_with_checksum[:-checksum_length]
    checksum = bin(
        int.from_bytes(message_with_checksum[-checksum_length:], byteorder="big")
    )[2:].zfill(
        16
    )  # checksum extraído
